import { Blog } from "@/types/blog";

const blogData: Blog[] = [
  {
    "id": "1",
    "title": "मां बगलामुखी मंदिर, नलखेड़ा: चमत्कारी शक्ति और आस्था का केंद्र",
    "video": "/videos/blog/blog-1.mp4",
    "thumbnail": "/images/blog/blog-1.jpg",
    "author": {
      "name": "Baglamukhi Nandkishor",
      "image": "/images/baglamukhi-nandkishor.jpg",
      "designation": "Spiritual Guide"
    },
    "tags": ["Baglamukhi Temple", "Spirituality", "Hinduism"],
    "publishDate": "Feb 19, 2025",
    "category": "Religious Tourism",
    "readTime": 5,
    "sections": [
      {
        "heading": "परिचय",
        "content": "मध्यप्रदेश के आगर मालवा जिले के नलखेड़ा में स्थित मां बगलामुखी मंदिर शक्ति उपासकों के लिए एक पवित्र स्थल है।"
      },
      {
        "heading": "मंदिर का इतिहास",
        "content": "यह मंदिर महाभारत काल से जुड़ा माना जाता है। पांडवों ने भगवान श्रीकृष्ण की सलाह पर यहां अनुष्ठान किए थे।"
      }
    ]
  },
  {
    "id": "2",
    "title": "श्मशान में है यह प्रसिद्ध मंदिर...",
    "video": "/videos/blog/blog-2.mp4",
    "thumbnail": "/images/blog/blog-2.jpg",
    "author": {
      "name": "Baglamukhi Nandkishor",
      "image": "/images/baglamukhi-nandkishor.jpg",
      "designation": "Spiritual Guide"
    },
    "tags": ["Baglamukhi Temple", "Mystical Places", "Hinduism"],
    "publishDate": "Feb 19, 2025",
    "category": "Religious Tourism",
    "readTime": 4,
    "sections": [
      {
        "heading": "मंदिर का महत्व",
        "content": "माता बगलामुखी का यह मंदिर नलखेड़ा कस्बे में लखुन्दर नदी के किनारे स्थित है।"
      },
      {
        "heading": "त्रिशक्ति देवी",
        "content": "यह मन्दिर तीन मुखों वाली त्रिशक्ति बगलामुखी देवी को समर्पित है।"
      }
    ]
  },
  {
    "id": "3",
    "title": "हिमाचल के इस मंदिर में होते हैं विशेष अनुष्ठान...",
    "video": "/videos/blog/blog-3.mp4",
    "thumbnail": "/images/blog/blog-3.jpg",
    "author": {
      "name": "Baglamukhi Nandkishor",
      "image": "/images/baglamukhi-nandkishor.jpg",
      "designation": "Spiritual Guide"
    },
    "tags": ["Baglamukhi Temple", "Kangra Temple", "Spiritual Practices"],
    "publishDate": "Feb 19, 2025",
    "category": "Religious Tourism",
    "readTime": 6,
    "sections": [
      {
        "heading": "मंदिर की विशेषता",
        "content": "कांगड़ा जिले के इस मंदिर में शत्रुनाशिनी यज्ञ किए जाते हैं।"
      },
      {
        "heading": "ग्रहों की शांति के लिए उपाय",
        "content": "इस मंदिर में घरेलू कलह, अदालती परेशानियों और अन्य समस्याओं से मुक्ति के लिए विशेष पूजा कराई जाती है।"
      }
    ]
  }
]
